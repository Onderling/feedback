// BasisChannelAdapter — the "pre-send" surface (architecture §1.3). Unlike Telegram,
// basis runs ON the participant's device with browser-held keys, so the floor runs
...
export class BasisChannelAdapter {
  ...
  constructor({ bridge, chatId, strings = getStrings() }) {
    if (!bridge || typeof bridge.sendReply !== 'function') throw new Error('BasisChannelAdapter: bridge with sendReply() required');
    if (!chatId) throw new Error('BasisChannelAdapter: chatId required');
    this.#bridge = bridge;
  ...
  // On basis this runs on-device (browser), before the message leaves.
